...
export default function SpeakerTable({ data, onRefresh }: { data: any[]; onRefresh: () => void }) {
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-slate-800 bg-slate-900/40 text-[11px] uppercase font-bold tracking-wider text-slate-400">
            <th className="p-4">Acoustic Speaker</th>
            <th className="p-4">Brand</th>
            <th className="p-4">Condition</th>
            <th className="p-4">Battery Life</th>
        ...
        <tbody className="divide-y divide-slate-900/60 text-sm">
          {data.map((item) => (
            <tr key={item.id} className="hover:bg-slate-900/20 text-slate-300">
              ...
              <td className="p-4 text-blue-400 font-medium">{item.batteryLife || "AC Wire"}</td>
      ...
      {editingItem && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0A0F1D] border border-slate-800 rounded-2xl max-w-md w-full p-6">
            <h3 className="text-md font-bold text-white mb-4 flex items-center gap-2"><Volume2 className="w-4 h-4 text-rose-500" /> Edit Speaker</h3>
