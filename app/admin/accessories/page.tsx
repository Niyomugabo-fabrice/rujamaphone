...
import { RefreshCw, Plus, Layers, Image as ImageIcon, X, Eye, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function AccessoriesPage() {
  ...
  const fetchAccessories = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await adminFetch("/api/accessories", { cache: "no-store" });
  ...
  const handleCreateAccessory = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      ...
      const response = await adminFetch("/api/accessories", {
        method: "POST",
        body: nativeFormData,
      });
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 relative text-slate-800">
      ...
      <div className="flex items-center justify-between border-b border-red-200/60 pb-5">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight uppercase">Accessories Catalog</h1>
          <p className="text-xs text-red-900/60 mt-0.5 font-medium">Hardware Matrix Components Storage Records</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="p-16 border border-dashed border-red-200/80 rounded-2xl text-center text-red-900/40 text-sm font-semibold bg-white/20">
          No accessories records discovered.
        </div>
      ) : (
        <AccessoryTable
          data={itemsData}
          ...
          <div className="bg-white border border-red-100 rounded-2xl max-w-md w-full p-6 text-slate-800 shadow-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                <Layers className="w-4 h-4 text-[#D90429]" /> Register New Accessory
              </h3>
              <button
                type="button"
                onClick={() => {
            ...
            <form onSubmit={handleCreateAccessory} className="space-y-4">
              <div>
                <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1 tracking-wider">Accessory Name</label>
                <input
                  type="text"
                  name="name"
          ...
          <div className="bg-white border border-red-100 rounded-2xl max-w-lg w-full p-6 text-slate-800 shadow-2xl relative animate-in zoom-in-95 duration-150">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-red-100">
                ...
                <div>
                  <span className="block text-[9px] uppercase font-bold text-slate-400 tracking-wider">Type</span>
                  <span className="text-sm font-bold text-slate-800">{selectedProduct.type}</span>
                </div>
                <div>
