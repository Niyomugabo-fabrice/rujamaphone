...
import { idSchema, productImagesSchema, speakerFormSchema } from "@/lib/schemas";
...
export async function GET(
  ...
  try {
    ...
    const item = await prisma.speaker.findUnique({ where: { id }, select: speakerSelect });
    return item ? ok(item) : fail("Speaker not found", 404);
  } catch (error) {
    return handleApiError("speakers.id.GET", error);
  }
}
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      const data = await parseJson(
        request,
        speakerFormSchema.partial().extend({ image: productImagesSchema.optional() }).refine(
    ...
    return handleApiError("speakers.id.PATCH", error);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return ok({ message: "Speaker record deleted" });
  } catch (error) {
    return handleApiError("speakers.id.DELETE", error);
  }
}
