...
import { accessoryFormSchema, accessoryQuerySchema, deleteByIdQuerySchema } from "@/lib/schemas";
...
export async function GET(request: Request) {
  try {
    ...
    const { page, limit, search, brand, condition, type, minPrice, maxPrice } =
    ...
    return ok({ data: accessories, total, page, totalPages: Math.max(Math.ceil(total / limit), 1) });
  } catch (error) {
